"""Tests for task service."""
import pytest
from unittest.mock import Mock, MagicMock
from datetime import datetime

from app.services.task_service import TaskService
from app.schemas.task import TaskCreate, TaskUpdate
from app.models.task import Task


class TestTaskService:
    """Test cases for TaskService."""
    
    @pytest.fixture
    def mock_repository(self):
        """Create a mock repository."""
        return Mock()
    
    @pytest.fixture
    def task_service(self, mock_repository):
        """Create a task service with mock repository."""
        return TaskService(mock_repository)
    
    def test_create_task(self, task_service, mock_repository):
        """Test creating a task."""
        # Arrange
        task_data = TaskCreate(title="Test Task", description="Test Description")
        expected_task = Task(
            id=1,
            title="Test Task",
            description="Test Description",
            completed=False,
            created_at=datetime.now(),
            updated_at=datetime.now()
        )
        mock_repository.create.return_value = expected_task
        
        # Act
        result = task_service.create_task(task_data)
        
        # Assert
        assert result == expected_task
        mock_repository.create.assert_called_once_with(task_data)
    
    def test_get_tasks(self, task_service, mock_repository):
        """Test getting all tasks."""
        # Arrange
        expected_tasks = [
            Task(id=1, title="Task 1", completed=False),
            Task(id=2, title="Task 2", completed=True)
        ]
        mock_repository.get_all.return_value = expected_tasks
        
        # Act
        result = task_service.get_tasks(skip=0, limit=10)
        
        # Assert
        assert result == expected_tasks
        mock_repository.get_all.assert_called_once_with(skip=0, limit=10)
    
    def test_get_task(self, task_service, mock_repository):
        """Test getting a single task."""
        # Arrange
        task_id = 1
        expected_task = Task(id=task_id, title="Test Task", completed=False)
        mock_repository.get_by_id.return_value = expected_task
        
        # Act
        result = task_service.get_task(task_id)
        
        # Assert
        assert result == expected_task
        mock_repository.get_by_id.assert_called_once_with(task_id)
    
    def test_update_task(self, task_service, mock_repository):
        """Test updating a task."""
        # Arrange
        task_id = 1
        task_update = TaskUpdate(title="Updated Task", completed=True)
        expected_task = Task(id=task_id, title="Updated Task", completed=True)
        mock_repository.update.return_value = expected_task
        
        # Act
        result = task_service.update_task(task_id, task_update)
        
        # Assert
        assert result == expected_task
        mock_repository.update.assert_called_once_with(task_id, task_update)
    
    def test_delete_task(self, task_service, mock_repository):
        """Test deleting a task."""
        # Arrange
        task_id = 1
        mock_repository.delete.return_value = True
        
        # Act
        result = task_service.delete_task(task_id)
        
        # Assert
        assert result is True
        mock_repository.delete.assert_called_once_with(task_id)